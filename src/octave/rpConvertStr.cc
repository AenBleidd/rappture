 ...
 *
 *    [retStr,err] = rpConvertStr(fromVal, toUnitsName, showUnits)
 *
 * ======================================================================
...
/**********************************************************************/
// METHOD: [retStr,err] = rpConvertStr(fromVal,toUnitsName,showUnits)
...
DEFUN_DLD (rpConvertStr, args, ,
"-*- texinfo -*-\n\
[retStr,result] = rpConvertStr(@var{fromVal},@var{toUnitsName},@var{showUnits})\n\
\n\
Convert the value and units in the string @var{fromVal} to units specified\n\
in string @var{toUnitsName}. If @var{showUnits} is set to 1, then show the\n\
units in the returned string @var{retStr}, else leave the units off.\n\
...
Error code, err=0 on success, anything else is failure.")
{
    static std::string who = "rpConvertStr";
