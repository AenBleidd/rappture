                        ...
                        int datatype, int len, int maxormin )
{
    ...
    /* gekco purify ctx = (PGAContext *) malloc ( sizeof(PGAContext) ); */
    ctx = (PGAContext *) calloc ( 1, sizeof(PGAContext) );
     ...
     *  EHW Addition             *
     *****************************/
    ctx->cops.SetTemp           = NULL;
    ctx->cops.EndOfRepFcn       = NULL;
    ctx->cops.DestroyUD         = NULL;
    ctx->cops.InitUserCtx       = NULL;
    ctx->cops.KillUserCtx       = NULL;
     ...
     *  EHW Addition             *
     *****************************/
    ctx->fops.SetTemp           = NULL;
    ctx->fops.EndOfRepFcn       = NULL;
    ctx->fops.DestroyUD         = NULL;
    ctx->fops.InitUserCtx       = NULL;
    ctx->fops.KillUserCtx       = NULL;


    /* Parallel */
    ctx->par.NumIslands        = PGA_UNINITIALIZED_INT;
    ctx->par.NumDemes          = PGA_UNINITIALIZED_INT;
    ctx->par.DefaultComm       = (MPI_Comm) NULL;
#ifdef FAKE_MPI
...
/* par */
    ...
    if ( ctx->par.NumDemes         == PGA_UNINITIALIZED_INT)
         ctx->par.NumDemes          = 1;
    if ( ctx->par.DefaultComm      == (MPI_Comm) NULL )
...
void PGACreatePop (PGAContext *ctx, int pop)
{
    ...
    PGADebugEntered("PGACreatePop");
     ...
     PGACreateIndividual (ctx, PGA_TEMP3, pop, PGA_FALSE);
