...
void PGARunMutationAndCrossover (PGAContext *ctx, int oldpop, int newpop)
{
    ...
    /* comment out by Lu Yang: PGASortPop( ctx, oldpop ); */
/*********************************************/
/* new added by Lu Yang */
    if ( ctx->cops.SortPop ) {
       (*ctx->cops.SortPop)(ctx, oldpop);
    } else if ( ctx->fops.SortPop ) {
        ( *ctx->fops.SortPop )( &ctx, &oldpop ) ;
    } else {
...
void PGAUpdateGeneration(PGAContext *ctx, MPI_Comm comm)
{
    ...
    if (rank == 0) {
      ...
      ctx->ga.iter--;
      /********  EHW Modification ********************/
      if(ctx->cops.SetTemp != NULL)
	(*ctx->cops.SetTemp)(ctx);
      if(ctx->fops.SetTemp != NULL)
	(*ctx->fops.SetTemp)(&ctx);
...
#define GEKCO_RESTART
#ifdef GEKCO_RESTART
/* get the number of identical evaluations no matter what
the stopping criterion is.  That number will be used for the restart. */
