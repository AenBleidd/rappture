...
option add *Videospeed.width 5 widgetDefault
option add *Videospeed.textBackground white widgetDefault

blt::bitmap define Videospeed-up {
#define up_width 8
#define up_height 4
...
blt::bitmap define Videospeed-down {
...
# CONSTRUCTOR
# ----------------------------------------------------------------------
itcl::body Rappture::Videospeed::constructor {args} {
    ...
    itk_component add up {
        button $itk_component(controls).spinup -bitmap Videospeed-up \
    ...
    itk_component add down {
        button $itk_component(controls).spindn -bitmap Videospeed-down \
            -borderwidth 1 -relief raised -highlightthickness 0 \
...
# sends a <<Value>> event.
# ----------------------------------------------------------------------
itcl::body Rappture::Videospeed::value {args} {
    if {[llength $args] == 1} {
        set string [lindex $args 0]
        # allow for floating point values
        if { [regexp {^ *([0-9]+\.?[0-9]*|\.[0-9]+)x *$} $string match newval] } {
        } elseif { [regexp {^ *([0-9]+\.?[0-9]*|\.[0-9]+) *$} $string match newval] } {
        } else {
            bell
            return
        }
...
# <factor> can be an integer value or the keyword "up" or "down".
# ----------------------------------------------------------------------
itcl::body Rappture::Videospeed::bump {{factor up}} {
    if {"up" == $factor} {
        set factor $itk_option(-factor)
    } elseif {"down" == $factor} {
        set factor [expr {1.0/$itk_option(-factor)}]
    } elseif {![string is integer $factor]} {
        error "bad factor \"$factor\": should be up, down, or integer"
    }
...
# ----------------------------------------------------------------------
itcl::body Rappture::Videospeed::_validate {char} {
