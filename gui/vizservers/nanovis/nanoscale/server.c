...
// justify redirection.
#define LOAD_REDIRECT_FACTOR 0.5
...
int find_best_host(void)
{
  ...
  for(h=0; h<sizeof(host_array)/sizeof(host_array[0]); h++) {
    if (host_array[h].in_addr.s_addr != 0 && host_array[h].load < best) {
      index = h;
...
int main(int argc, char *argv[])
{
  ...
  while(1) {
    ...
    if (FD_ISSET(send_fd, &rfds)) {
      ...
      printf("Load for %s is %f.\n",
             inet_ntoa(peer_addr.sin_addr), peer_load);
      int h;
      int free_index=-1;
      int found = 0;
      for(h=0; h<sizeof(host_array)/sizeof(host_array[0]); h++) {
