	      ...
	      MPI_Comm comm)
{
    ...
    /* EHW: Starting the Initialization of the User Context Data */
    if (rank == 0){
      if (ctx->fops.InitUserCtx)
	(*ctx->fops.InitUserCtx)(&ctx);
      if (ctx->cops.InitUserCtx)
	(*ctx->cops.InitUserCtx)(ctx);
    }

    PGAEvaluate(ctx, PGA_OLDPOP, f, comm);
    if (rank == 0){
      int popindex = PGA_OLDPOP ;
      if ( ctx->cops.Fitness ) {
          (*ctx->cops.Fitness)(ctx, PGA_OLDPOP);
      }
      else if ( ctx->fops.Fitness ) {
          (*ctx->fops.Fitness)(&ctx,&popindex) ;
      }  
      else {
          PGAFitness(ctx, PGA_OLDPOP);
      }

      /* EHW: Running the endof generation function so that the
	 initial population is saved */
	...
	if (rank == 0) {
		...
		PGARestart(ctx, PGA_OLDPOP, PGA_NEWPOP);
                ...
                int popindex = PGA_OLDPOP ;
                if ( ctx->cops.Select ) {
                    (*ctx->cops.Select)(ctx, PGA_OLDPOP) ;
                } else if ( ctx->fops.Select ) {
                    (*ctx->fops.Select)(&ctx, &popindex) ;
                } else {
	...
	if (rank == 0) {
            int popindex = PGA_NEWPOP ; 
            if ( ctx->cops.Fitness ) {
                (*ctx->cops.Fitness)(ctx, PGA_NEWPOP);
            } else if ( ctx->fops.Fitness ) {
                ( *ctx->fops.Fitness )( &ctx, &popindex ) ;
            } else {
	...
	/* EHW: Destroying the User Context Data */
	if (ctx->fops.KillUserCtx)
	  (*ctx->fops.KillUserCtx)(&ctx);
	if (ctx->cops.KillUserCtx)
	  (*ctx->cops.KillUserCtx)(ctx);
		   ...
		   double (*f)(PGAContext *c, int p, int pop), MPI_Comm comm)
{
	...
	/*  Receive the next evaluated string.  */
        ...
        PGAReceiveIndividual(ctx, PGA_TEMP3, pop, MPI_ANY_SOURCE,
			   PGA_COMM_STRINGTOEVAL,  comm, &stat);
	p = work[stat.MPI_SOURCE];

	PGASetEvaluation(ctx,p,pop,PGAGetEvaluation(ctx,PGA_TEMP3,pop));
	(*ctx->cops.CopyString)(ctx,PGA_TEMP3,pop,p,pop);
		      ...
		      double (*f)(PGAContext *, int, int), MPI_Comm comm)
{
	...
	PGASetEvaluation(ctx,PGA_TEMP1,pop,e);
	PGASendIndividual(ctx, PGA_TEMP1, pop, 0 ,
			  PGA_COMM_STRINGTOEVAL, comm);
