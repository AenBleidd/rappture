...
R2Fonts::R2Fonts()
: _fontIndex(-1), _screenWidth(512), _screenHeight(512)
{
}
...
void R2Fonts::setFont(const char* fontName)
{
	for (R2int32 index = 0; index < _fonts.size(); ++index)
	{
...
void R2Fonts::addFont(const char* fontName, const char* fontFileName)
{
...
void R2Fonts::draw(const char* pString, ...) const
{
	va_list vlArgs;
    ...
    if (_fontIndex != -1)
    {
        ...
        glCallLists(length, GL_UNSIGNED_BYTE, reinterpret_cast<const GLvoid*>(szVargsBuffer));
    }
}
...
void R2Fonts::initializeFont(R2FontAttributes& attr)
{
	...
	for (index = 0; index < 256; ++index)
	{
		...
		if (charInfo._valid)
		{
			...
			glVertex2i(0.0f,  attr._fontHeight);

			glTexCoord2f(charInfo._right, charInfo._top);
			glVertex2i(charInfo._width, 0.0f);

			glTexCoord2f(charInfo._right,  charInfo._bottom);
			glVertex2i(charInfo._width, attr._fontHeight);
...
R2bool R2Fonts::loadFont(const char* fontName, const char* fontFileName, R2FontAttributes& sFont)
{
    ...
    if (fsInput)
    {
        ...
        if (uiFileId == c_nFileMagicHeader)
        {
            ...
            fsInput.read(reinterpret_cast<char*>(&sFont._textureWidth), sizeof(unsigned int));
            fsInput.read(reinterpret_cast<char*>(&sFont._textureHeight), sizeof(unsigned int));
            fsInput.read(reinterpret_cast<char*>(&sFont._fontHeight), sizeof(unsigned int));
            ...
            !fsInput.fail( ))
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, sFont._textureWidth, sFont._textureHeight, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, pTexMap);
...
void R2Fonts::resize(R2int32 width, R2int32 height)
{
	_screenWidth = width;
