...
void PGADestroy (PGAContext *ctx)
{
    ...
    if (ctx->sys.SetUpCalled == PGA_TRUE) {
      ...
      for ( i = 0; i < (ctx->ga.PopSize + 3); i++ ) {
/**********************  EHW Project Modifications *******************/
	PGADestroyUD(ctx,i,PGA_OLDPOP);
	PGADestroyUD(ctx,i,PGA_NEWPOP);
/**********************  EHW Project Modifications *******************/	
...
/************* EHW ADDITION **********/
/************  It destroys the User Data *******/
void PGADestroyUD(PGAContext *ctx, int p,int pop)
{
  PGAIndividual *ind;

 
  if(ctx->cops.DestroyUD == NULL)
  { /* Conventional Destruction */
    ind=PGAGetIndividual(ctx,p,pop);
    free(ind->UserData); 
  }else{
    (ctx->cops.DestroyUD)(ctx,p,pop);
  }
