...
vtkStandardNewMacro(vtkRpCubeAxesActor2D);

vtkCxxSetObjectMacro(vtkRpCubeAxesActor2D,Input, vtkDataSet);
vtkCxxSetObjectMacro(vtkRpCubeAxesActor2D,Camera,vtkCamera);
vtkCxxSetObjectMacro(vtkRpCubeAxesActor2D,ViewProp,vtkProp);
vtkCxxSetObjectMacro(vtkRpCubeAxesActor2D,AxisLabelTextProperty,vtkTextProperty);
vtkCxxSetObjectMacro(vtkRpCubeAxesActor2D,AxisTitleTextProperty,vtkTextProperty);

//----------------------------------------------------------------------------
// Instantiate this object.
vtkRpCubeAxesActor2D::vtkRpCubeAxesActor2D()
{
  this->Input = NULL;
  this->ViewProp = NULL;
...
void vtkRpCubeAxesActor2D::ShallowCopy(vtkRpCubeAxesActor2D *actor)
{
...
int vtkRpCubeAxesActor2D::RenderOverlay(vtkViewport *viewport)
{
...
// cube. Determine which parts of the edges are visible via intersection 
// with the boundary of the viewport (minus borders).
int vtkRpCubeAxesActor2D::RenderOpaqueGeometry(vtkViewport *viewport)
{
  double bounds[6];
...
int vtkRpCubeAxesActor2D::HasTranslucentPolygonalGeometry()
{
  return 0;
}
...
void vtkRpCubeAxesActor2D::AdjustAxes(double pts[8][3], double bounds[6],
...
void vtkRpCubeAxesActor2D::ReleaseGraphicsResources(vtkWindow *win)
{
...
//----------------------------------------------------------------------------
// Return the ranges
void vtkRpCubeAxesActor2D::GetRanges(double ranges[6])
{
  int i;
  for ( i=0; i<6; i++ )
    {
...
//----------------------------------------------------------------------------
// Compute the ranges
void vtkRpCubeAxesActor2D::GetRanges(double& xmin, double& xmax, 
...
//----------------------------------------------------------------------------
// Compute the bounds
double *vtkRpCubeAxesActor2D::GetRanges()
{
  double ranges[6];
  this->GetRanges(ranges);
  return this->Ranges;
...
//----------------------------------------------------------------------------
// Compute the bounds
void vtkRpCubeAxesActor2D::GetBounds(double bounds[6])
{
  double *propBounds;
  int i;
...
//----------------------------------------------------------------------------
// Compute the bounds
void vtkRpCubeAxesActor2D::GetBounds(double& xmin, double& xmax, 
                                     double& ymin, double& ymax,
                                     double& zmin, double& zmax)
{
  double bounds[6];
...
//----------------------------------------------------------------------------
// Compute the bounds
double *vtkRpCubeAxesActor2D::GetBounds()
{
...
//----------------------------------------------------------------------------
void vtkRpCubeAxesActor2D::PrintSelf(ostream& os, vtkIndent indent)
{
...
#define VTK_DIVS 10
int vtkRpCubeAxesActor2D::ClipBounds(vtkViewport *viewport, double pts[8][3], 
                                     double bounds[6])
{
  int i, j, k, numIters;
  double planes[24];
...
//----------------------------------------------------------------------------
void vtkRpCubeAxesActor2D::TransformBounds(vtkViewport *viewport, 
...
double vtkRpCubeAxesActor2D::EvaluatePoint(double planes[24], double x[3])
{
  int kk;
  double *plane;
...
// frustum planes.
double vtkRpCubeAxesActor2D::EvaluateBounds(double planes[24], double bounds[6])
{
