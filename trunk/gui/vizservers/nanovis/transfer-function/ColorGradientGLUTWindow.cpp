...
void ColorGradientGLUTWindow::cmInit(int main_win_x, int main_win_y)
{
...
void ColorGradientGLUTWindow::createGLUIWidgets(){
	cm_glui = GLUI_Master.create_glui_subwindow(colorMapWindow, GLUI_SUBWINDOW_BOTTOM);
...
void ColorGradientGLUTWindow::changeState(int arg){
	switch (arg){
	case 1: //add color 
		cm_editState=1;
		break;
...
void ColorGradientGLUTWindow::cmDisplay()
{
...
void ColorGradientGLUTWindow::cmReshape(int x, int y)
{
	//printf("cmReshape");
	cm_winx=x;
	cm_winy=100;
...
void ColorGradientGLUTWindow::cmDestroy(){}



void ColorGradientGLUTWindow::cmKeyboard(unsigned char key, int x, int y){
	switch(key) 
	{    
	case 'p':
		//printPoints();
...
void ColorGradientGLUTWindow::cmIdle(){ }



void ColorGradientGLUTWindow::cmMouse(int button, int state, int x, int y)
{	
	//printf("cm mouse\n");
...
void ColorGradientGLUTWindow::cmMotion(int x, int y)
{
	GLint viewportVector[4];
	double scX, scY;
...
bool ColorGradientGLUTWindow::SelectPoint(double x, double y){
...
void ColorGradientGLUTWindow::WriteControlPoints()
{
...
ControlPoint* ColorGradientGLUTWindow::boundaryChecking(){
...
void ColorGradientGLUTWindow::sortPoints(){
	if (map->numOfKeys==0)
		return;
...
void ColorGradientGLUTWindow::printInterpolation(){
	int i=0;
	for(i=0;i<numOfOutput;i++){
