...
# ----------------------------------------------------------------------
#  COMPONENT: MultiChoiceEntry - widget for entering a choice of strings
#
#  This widget represents a <multichoice> entry on a control panel.
...
itcl::class Rappture::MultiChoiceEntry {
    inherit itk::Widget
...
# CONSTRUCTOR
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::constructor {owner path args} {
    ...
    itk_component add choice {
        Rappture::Combochecks $itk_interior.choice -editable no \
...
# DESTRUCTOR
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::destructor {} {
...
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::value {args} {
    set onlycheck 0
    ...
    regsub -all {, } $str {,} str
    foreach lbl [split $str {,}] {
        if {[info exists _str2val($lbl)]} {
            set lbl $_str2val($lbl)
        }
        lappend result $lbl
    }
    return [join $result {, }]
...
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::label {} {
    set label [string trim [$_owner xml get $_path.about.label]]
    if {"" == $label} {
        set label "MultiChoice"
    }
    return $label
}
...
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::tooltip {} {
...
# widget.
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::_rebuild {} {
    ...
    #
    set defvallist [$_owner xml get $_path.default]
    if {"" != $defvallist} {
        set v {}
        foreach defval [split $defvallist {,}] {
...
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::_newValue {} {
...
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::_tooltip {} {
...
# ----------------------------------------------------------------------
itcl::body Rappture::MultiChoiceEntry::_log {} {
...
# ----------------------------------------------------------------------
itcl::configbody Rappture::MultiChoiceEntry::state {
