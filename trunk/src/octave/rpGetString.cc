 ...
 *
 *    [retStr,err] = rpGetString(libHandle,path)
 *
 * ======================================================================
...
/**********************************************************************/
// METHOD: [retStr,err] = rpGetString(libHandle,path)
...
DEFUN_DLD (rpGetString, args, ,
"-*- texinfo -*-\n\
[retStr,err] = rpGetString(@var{libHandle},@var{path})\n\
\n\
Clients use this to query the value of a node.  If the path\n\
is not specified, it returns the value associated with the\n\
root node.  Otherwise, it returns the value for the element\n\
...
Error code, err=0 on success, anything else is failure.")
{
    static std::string who = "rpGetString";
