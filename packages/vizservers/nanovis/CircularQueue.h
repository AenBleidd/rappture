...
template<typename Element, unsigned int Size>
bool CircularFifo<Element, Size>::top(Element& data)
{
   if(head == tail)
...
bool CircularFifo<Element, Size>::isFull() const
{
   ...
   return (((unsigned int) tailCheck) == head);
}
...
template<typename Element, unsigned int Size>
int CircularFifo<Element, Size>::tailIndex()
{
   int nextTail = increment(tail);
   if(((unsigned int)nextTail) != head)
   {
...
template<typename Element, unsigned int Size>
bool CircularFifo<Element, Size>::push()
{
   int nextTail = increment(tail);
   if(((unsigned int)nextTail) != head)
   {
